...
do_init(ZoneName, BindIp) ->
    ...
    Upstream = application:get_env(erlkoenig, dns_upstream, {8, 8, 8, 8}),
    Domain   = list_to_binary(
                 application:get_env(erlkoenig, dns_domain, "erlkoenig")),
    TTL      = application:get_env(erlkoenig, dns_ttl, 5),
