...
init(Opts) ->
    ...
    _ = ensure_ets(?CT_TAB, [named_table, set, public, {read_concurrency, true}]),
    %% Aggregation: {SrcIP, Count, FirstSeen, LastSeen}
    _ = ensure_ets(?CT_AGG, [named_table, set, public, {read_concurrency, true}]),
...
handle_new(
    ...
    erlkoenig_nft_events:notify_ct({ct_new, Event}),
        ...
        full ->
            ...
            case Size >= Max of
                true ->
                    ...
                    erlkoenig_nft_events:notify_ct({ct_alert, {mode_switch, aggregate}}),
...
handle_destroy(#{src := _} = Event, #{ct_destroy := N, mode := Mode} = State) ->
    erlkoenig_nft_events:notify_ct({ct_destroy, Event}),
