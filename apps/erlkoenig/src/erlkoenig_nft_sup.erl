...
-moduledoc """
Top-level supervisor for the Erlkoenig firewall.

Uses rest_for_one strategy. Children are ordered by dependency.
This tree contains ONLY firewall-critical components.
...
Started as a child of erlkoenig_sup. Firewall crashes don't affect
containers, and container crashes don't affect the firewall.
...
init([]) ->
    ...
    Children = [
        ...
        %% JSON API socket removed — all commands go through erlkoenig_ctl
    ],
