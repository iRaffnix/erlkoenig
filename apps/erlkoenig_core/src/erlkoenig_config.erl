...
validate_config(#{containers := Containers}) when is_list(Containers) ->
    validate_containers(Containers);
validate_config(#{containers := _}) ->
    {error, {invalid_type, containers, expected_list}};
validate_config(_) ->
    %% Config without containers is valid (may only have watches/guard)
    ok.
...
apply_config(Config) ->
    ...
    Watches = maps:get(watches, Config, []),
    Results = lists:filtermap(fun spawn_container/1, Containers),
    lists:foreach(fun start_watch/1, Watches),
...
spawn_container(#{name := Name, binary := Binary} = Ct) ->
    ...
    case erlkoenig_core:spawn(BinPath, SpawnOpts) of
        {ok, Pid} ->
...
container_names(#{containers := Containers}) ->
    [iolist_to_binary(maps:get(name, C)) || C <- Containers];
container_names(_) ->
    [].
