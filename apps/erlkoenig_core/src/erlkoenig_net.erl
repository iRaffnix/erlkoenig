...
%%%-------------------------------------------------------------------
%% @doc erlkoenig_net - Container network orchestration.
%%
...
%% @doc Set up networking for a container.
%%
...
-spec setup_container_net(port(), binary(), non_neg_integer()) ->
    {ok, map()} | {error, term()}.
setup_container_net(Port, ContainerId, OsPid) ->
    setup_container_net(Port, ContainerId, OsPid, default).

%% @doc Set up networking: allocate IP from a named zone, or use an
%% explicit IP in the default zone.
-spec setup_container_net(port(), binary(), non_neg_integer(),
...
setup_container_net(Port, ContainerId, OsPid, Ip, ZoneName) ->
    ...
    case do_host_setup(HostVeth, ContainerVeth, OsPid, ZoneName) of
        ok ->
            Gateway = zone_gateway(ZoneName),
            Mask = zone_netmask(ZoneName),
            case do_netns_setup(Port, ContainerVeth, Ip, Gateway, Mask) of
                ok ->
...
%% @doc Tear down a container's network (veth + IP release).
%%
%% Deleting the host veth automatically deletes the peer.
...
%% @doc Tear down only the veth pair, keep the IP reserved.
%%
...
do_host_setup_veth(Sock, HostVeth, ContainerVeth, OsPid, ZoneName) ->
    ...
    case erlkoenig_netlink:request(
           Sock, erlkoenig_netlink:msg_create_veth(Seq, HostVeth, ContainerVeth)) of
        ok ->
            do_host_setup_netns(Sock, HostVeth, ContainerVeth, OsPid, ZoneName);
        {error, _} = Err ->
            Err
    end.

%% Step 2: Move container end into container's netns.
-spec do_host_setup_netns(socket:socket(), binary(), binary(),
                          non_neg_integer(), atom()) -> ok | {error, term()}.
do_host_setup_netns(Sock, HostVeth, ContainerVeth, OsPid, ZoneName) ->
    case get_ifindex(Sock, ContainerVeth) of
        {ok, PeerIdx} ->
            Seq = erlkoenig_netlink:next_seq(),
            case erlkoenig_netlink:request(
                   Sock, erlkoenig_netlink:msg_set_netns_by_pid(Seq, PeerIdx, OsPid)) of
                ok ->
                    do_host_setup_bridge(Sock, HostVeth, ZoneName);
                {error, _} = Err ->
...
-spec do_host_setup_bridge(socket:socket(), binary(), atom()) -> ok | {error, term()}.
do_host_setup_bridge(Sock, HostVeth, ZoneName) ->
    BridgeIdx = erlkoenig_bridge:ifindex(ZoneName),
    case get_ifindex(Sock, HostVeth) of
        {ok, HostIdx} ->
            Seq1 = erlkoenig_netlink:next_seq(),
            case erlkoenig_netlink:request(
                   Sock, erlkoenig_netlink:msg_set_master(Seq1, HostIdx, BridgeIdx)) of
                ok ->
                    %% Step 4: Bring host end up
                    Seq2 = erlkoenig_netlink:next_seq(),
                    case erlkoenig_netlink:request(
                           Sock, erlkoenig_netlink:msg_set_up(Seq2, HostIdx)) of
                        ok ->
                            ok;
                        {error, _} = Err ->
                            _ = delete_link(Sock, HostVeth),
...
-spec do_netns_setup(port(), binary(), inet:ip4_address(),
                     inet:ip4_address(), non_neg_integer()) -> ok | {error, term()}.
do_netns_setup(Port, IfName, Ip, Gateway, Prefixlen) ->
