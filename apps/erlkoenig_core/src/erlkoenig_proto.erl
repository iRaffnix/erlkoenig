...
%%%-------------------------------------------------------------------
%%% @doc Erlkoenig wire protocol encode/decode.
%%%
...
encode_handshake() -> <<?PROTOCOL_VERSION:8>>.

-spec check_handshake_reply(binary()) -> ok | {error, {protocol_mismatch, integer(), integer()}}.
check_handshake_reply(<<?PROTOCOL_VERSION:8>>) ->
    ok;
check_handshake_reply(<<Got:8>>) ->
...
%% @doc Encode CMD_SPAWN with capabilities (dns_ip defaults to 0).
%% CapsKeep: 64-bit bitmask, bit N = keep CAP_N (0 = drop all).
-spec encode_cmd_spawn(binary(), [binary()], [{binary(), binary()}],
                       non_neg_integer(), non_neg_integer(),
                       non_neg_integer(), non_neg_integer(),
                       non_neg_integer()) -> binary().
...
%% @doc Encode CMD_SPAWN with all options including flags.
%% DnsIp: 32-bit network-order IPv4 for /etc/resolv.conf (0 = default gateway).
%% Flags: 32-bit spawn flags (bit 0 = PTY mode).
-spec encode_cmd_spawn(binary(), [binary()], [{binary(), binary()}],
                       non_neg_integer(), non_neg_integer(),
                       non_neg_integer(), non_neg_integer(),
                       non_neg_integer(), non_neg_integer(),
