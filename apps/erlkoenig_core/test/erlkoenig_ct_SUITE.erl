...
init_per_suite(Config) ->
    ...
    application:ensure_all_started(erlkoenig_core),
    Config.
...
wait_stopped(Pid, Timeout) ->
    ...
    after Timeout ->
        ...
        case erlkoenig_core:inspect(Pid) of
            #{state := stopped} -> ok;
            _ -> timeout
        end
    end,
...
spawn_and_exit(_Config) ->
    ...
    {ok, Pid} = erlkoenig_core:spawn(BinPath),
    ?assert(is_pid(Pid)),
...
spawn_and_kill(_Config) ->
    ...
    {ok, Pid} = erlkoenig_core:spawn(BinPath),
    timer:sleep(100),
    erlkoenig_core:kill(Pid, 9),
    ok = wait_stopped(Pid, 5000).

spawn_bad_binary(_Config) ->
    {ok, Pid} = erlkoenig_core:spawn(<<"/nonexistent/binary">>),
    %% Should fail during creating (port can't spawn, or child fails execve)
    timer:sleep(1000),
    Res = erlkoenig_core:inspect(Pid),
    case Res of
