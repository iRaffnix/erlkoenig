...
normalize_fills_defaults_test_() ->
    ...
    {setup,
     fun() ->
         application:set_env(erlkoenig_core, zones,
             [{minimal, #{}}]),
         cleanup_zone_ets(),
...
setup_legacy() ->
    ...
    application:set_env(erlkoenig_core, subnet, {10, 0, 0, 0}),
    application:set_env(erlkoenig_core, gateway, {10, 0, 0, 1}),
    application:set_env(erlkoenig_core, bridge_name, <<"erlkoenig_br0">>),
    application:set_env(erlkoenig_core, netmask, 24),
    cleanup_zone_ets(),
    {ok, Pid} = erlkoenig_zone:start_link(),
    Pid.
