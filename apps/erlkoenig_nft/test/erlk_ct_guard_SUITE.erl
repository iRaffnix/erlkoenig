...
all() ->
    [flood_below_threshold, flood_at_threshold,
     flood_only_counts_within_window,
     port_scan_below_threshold, port_scan_at_threshold,
     port_scan_distinct_ports_only, already_banned_not_recounted,
     whitelist_tuple_format, whitelist_binary_format,
     whitelist_string_format, whitelisted_ip_not_banned,
     cleanup_old_events, cleanup_expired_bans].
...
flood_below_threshold(_) ->
    {C, _B} = make_tables(),
    IP = <<10,0,0,5>>, Now = erlang:system_time(second),
...
collect_ports(_, '$end_of_table', _, P) -> sets:to_list(P);
...
collect_ports(_, _, _, P) -> sets:to_list(P).

delete_before(_, '$end_of_table', _, Cnt) -> Cnt;
delete_before(C, {_, Ts, _} = K, Cutoff, Cnt) when Ts < Cutoff ->
    Next = ets:next(C, K), ets:delete(C, K), delete_before(C, Next, Cutoff, Cnt + 1);
delete_before(_, _, _, Cnt) -> Cnt.

cleanup_bans(B, Now) ->
    ets:foldl(fun({IP, _, Exp, _}, Cnt) ->
        case Exp =< Now of true -> ets:delete(B, IP), Cnt + 1; false -> Cnt end
    end, 0, B).
...
normalize_whitelist(L) ->
    lists:map(fun
        ...
        (Str) when is_list(Str) ->
            {ok, {A,B,C,D}} = inet:parse_ipv4_address(Str), <<A,B,C,D>>
    end, L).
