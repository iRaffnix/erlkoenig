 ...
 */
static inline int ek_bpf_map_create(enum bpf_map_type type,
				    uint32_t key_size,
				    uint32_t value_size,
...
static inline int ek_bpf_tracepoint_id(const char *group, const char *name)
{
	...
	for (size_t i = 0; i < sizeof(bases) / sizeof(bases[0]); i++) {
		ret = snprintf(path, sizeof(path), "%s/%s/%s/id",
			       bases[i], group, name);
 ...
 */
static inline int ek_bpf_attach_tracepoint_cpu(int prog_fd,
					       int tracepoint_id,
					       int cpu)
{
	...
	perf_fd = (int)syscall(__NR_perf_event_open, &pe,
			       -1 /* all pids */, cpu, -1 /* group */,
 ...
 */
static inline int ek_bpf_tp_field_offset(const char *group,
					  const char *tp_name,
					  const char *field_name)
{
	...
	for (size_t i = 0; i < sizeof(bases) / sizeof(bases[0]); i++) {
		ret = snprintf(path, sizeof(path), "%s/%s/%s/format",
			       bases[i], group, tp_name);
		if (ret < 0 || (size_t)ret >= sizeof(path))
			continue;
	...
	while (fgets(line, (int)sizeof(line), f)) {
...

		{
			...
			if (endptr == off_str + 7 || val < 0 || val > INT16_MAX) {
				fprintf(stderr,
					"bpf: field '%s' offset out of range: %ld\n",
					field_name, val);
				continue;
			}
			offset = (int)val;
