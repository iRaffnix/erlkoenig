...
static inline void erlkoenig_buf_init(struct erlkoenig_buf *b,
...
static inline int buf_read_bytes(struct erlkoenig_buf *b,
 ...
 */
static inline int buf_read_str16(struct erlkoenig_buf *b,
				 const uint8_t **out, uint16_t *len)
{
...
static inline int buf_read_str8(struct erlkoenig_buf *b,
				const uint8_t **out, uint8_t *len)
{
...
static inline int buf_read_bin16(struct erlkoenig_buf *b,
				 const uint8_t **out, uint16_t *len)
{
	return buf_read_str16(b, out, len);
}

static inline int buf_read_bin32(struct erlkoenig_buf *b,
...
static inline int buf_write_bytes(struct erlkoenig_buf *b,
...
static inline int buf_write_str16(struct erlkoenig_buf *b,
...
static inline int buf_write_str8(struct erlkoenig_buf *b,
...
static inline int buf_write_bin16(struct erlkoenig_buf *b,
				  const uint8_t *data, uint16_t len)
{
	return buf_write_str16(b, (const char *)data, len);
}

static inline int buf_write_bin32(struct erlkoenig_buf *b,
...
static inline int erlkoenig_write_frame(int fd, const uint8_t *buf, size_t len)
{
	...
	while (total > 0) {
		...
		if (consumed >= iov[0].iov_len) {
			...
			iov[0].iov_len = 0;
			iov[0].iov_base = (uint8_t *)iov[1].iov_base
					   + consumed;
			iov[0].iov_len = 0;
			iov[1].iov_base = (uint8_t *)iov[1].iov_base
					   + consumed;
			iov[1].iov_len -= consumed;
		} else {
			iov[0].iov_base = (uint8_t *)iov[0].iov_base
					   + consumed;
