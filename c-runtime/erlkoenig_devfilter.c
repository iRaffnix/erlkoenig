				  ...
				  size_t n_rules)
{
	...
	prog[pc++] = (struct bpf_insn){
		...
		.dst_reg = BPF_REG_3, .src_reg = BPF_REG_2,
		.off = 0, .imm = 0,
	};
	...
	for (size_t i = 0; i < n_rules; i++) {
		...
		if (r->type != 0) {
			if (pc >= max_insns) return -1;
			prog[pc++] = BPF_JMP_IMM(BPF_JNE, BPF_REG_2,
						  r->type, skip);
			skip--;
		}
		if (r->access != 0) {
			...
			if (r->access != EK_DEV_ACC_RWM) {
				...
				uint32_t forbidden = (~r->access) & EK_DEV_ACC_RWM;
				if (pc >= max_insns) return -1;
				prog[pc++] = BPF_JMP_IMM(BPF_JSET, BPF_REG_3,
							  (int32_t)forbidden,
							  skip);
				skip--;
			} else {
		...
		if (r->major != EK_DEV_WILDCARD) {
			if (pc >= max_insns) return -1;
			prog[pc++] = BPF_JMP_IMM(BPF_JNE, BPF_REG_4,
						  r->major, skip);
			skip--;
		}
		if (r->minor != EK_DEV_WILDCARD) {
			if (pc >= max_insns) return -1;
			prog[pc++] = BPF_JMP_IMM(BPF_JNE, BPF_REG_5,
						  r->minor, skip);
			skip--;
		}
			...
			size_t n_rules)
{
	...
	insn_cnt = generate_device_filter(prog, MAX_BPF_INSNS,
					  rules, n_rules);
	if (insn_cnt < 0) {
...
const struct ek_dev_rule ek_default_dev_rules[] = {
	...
	/* /dev/zero */
	{ .type = EK_DEV_CHAR, .major = 1, .minor = 5,
	  .access = EK_DEV_ACC_RWM },
	/* /dev/full */
	{ .type = EK_DEV_CHAR, .major = 1, .minor = 7,
	  .access = EK_DEV_ACC_RWM },
	/* /dev/random */
	{ .type = EK_DEV_CHAR, .major = 1, .minor = 8,
	  .access = EK_DEV_ACC_RWM },
	/* /dev/urandom */
	{ .type = EK_DEV_CHAR, .major = 1, .minor = 9,
	  .access = EK_DEV_ACC_RWM },
	/* /dev/tty */
	{ .type = EK_DEV_CHAR, .major = 5, .minor = 0,
	  .access = EK_DEV_ACC_RWM },
	/* /dev/ptmx */
	{ .type = EK_DEV_CHAR, .major = 5, .minor = 2,
	  .access = EK_DEV_ACC_RWM },
