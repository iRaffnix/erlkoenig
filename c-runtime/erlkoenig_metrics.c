				 ...
				 int parent_pid_off, int child_pid_off)
{
	...
	if (pc + 10 > max_insns) return -1;

	prog[pc++] = BPF_ST_MEM(BPF_B, BPF_REG_8,
				 (int16_t)offsetof(struct ek_metrics_event, type),
				 EK_METRICS_FORK);
	prog[pc++] = BPF_LDX_MEM(BPF_W, BPF_REG_1, BPF_REG_6,
				  (int16_t)parent_pid_off);
	prog[pc++] = BPF_STX_MEM(BPF_W, BPF_REG_8, BPF_REG_1,
				 ...
				 int pid_off)
{
	...
	prog[pc++] = BPF_MOV64_REG(BPF_REG_1, BPF_REG_8);
	prog[pc++] = BPF_ALU64_IMM(BPF_ADD, BPF_REG_1,
				 ...
				 int pid_off)
{
	...
	(void)pid_off; /* exit code not in tracepoint ctx; waitpid provides it */
	...
	prog[pc++] = BPF_STX_MEM(BPF_W, BPF_REG_8, BPF_REG_0,
				  (int16_t)offsetof(struct ek_metrics_event, tgid));
	prog[pc++] = BPF_ST_MEM(BPF_W, BPF_REG_8,
				 (int16_t)offsetof(struct ek_metrics_event, exit_ev.exit_code),
				 0);
	prog[pc++] = BPF_CALL_HELPER(BPF_FUNC_ktime_get_ns);
	prog[pc++] = BPF_STX_MEM(BPF_DW, BPF_REG_8, BPF_REG_0,
				  (int16_t)offsetof(struct ek_metrics_event, timestamp_ns));
...
static int ringbuf_mmap_setup(struct ek_metrics_ctx *ctx)
{
	...
	void *producer = mmap(NULL, producer_size, PROT_READ,
			      MAP_SHARED, ctx->ringbuf_fd, (off_t)page_size);
...
int ek_metrics_consume(struct ek_metrics_ctx *ctx,
		       ek_metrics_callback_t callback,
		       void *userdata)
{
	...
	unsigned long *consumer_pos_ptr =
		(unsigned long *)ctx->ring_mmap;

	/* Producer position: first 8 bytes of producer page (ring_producer) */
	unsigned long *producer_pos_ptr =
		(unsigned long *)ctx->ring_producer;

	/* Data starts after the producer page header */
	size_t page_size = (size_t)getpagesize();
	uint8_t *data_base =
		(uint8_t *)ctx->ring_producer + page_size;

	unsigned long cons_pos = atomic_load_explicit(
		(_Atomic unsigned long *)consumer_pos_ptr,
		memory_order_acquire);

	unsigned long prod_pos = atomic_load_explicit(
		(_Atomic unsigned long *)producer_pos_ptr,
		memory_order_acquire);
	...
	while (cons_pos < prod_pos) {
		...
		uint32_t len = atomic_load_explicit(
			(_Atomic uint32_t *)hdr,
			memory_order_acquire);
		...
		uint32_t record_len = len & ~(BPF_RINGBUF_BUSY_BIT |
...
static const struct tp_info tracepoints[EK_METRICS_N_PROGS] = {
	...
	[EK_METRICS_PROG_OOM]  = { "oom",   "mark_victim",
				    EK_METRICS_PROG_OOM },
};
...
static int attach_program(struct ek_metrics_ctx *ctx, int prog_idx)
{
	...
	if (tp_id < 0) {
		LOG_WARN("metrics: tracepoint %s/%s not available",
			 tp->group, tp->name);
		return tp_id;
	}
	...
	for (int cpu = 0; cpu < ctx->n_cpus; cpu++) {
		...
		if (perf_fd < 0) {
			...
			LOG_DBG("metrics: attach %s/%s cpu %d: %s",
				tp->group, tp->name, cpu,
				strerror(-perf_fd));
	...
	if (attached == 0) {
		LOG_WARN("metrics: %s/%s: no CPUs attached",
			 tp->group, tp->name);
		return -ENODEV;
	}

	LOG_INFO("metrics: attached %s/%s on %d/%d CPUs",
		 tp->group, tp->name, attached, ctx->n_cpus);
	return 0;
}
...
int ek_metrics_start(const char *cgroup_path, struct ek_metrics_ctx *ctx)
{
	...
	ctx->cgroup_map_fd = ek_bpf_map_create(BPF_MAP_TYPE_ARRAY,
						sizeof(uint32_t),
						sizeof(uint64_t), 1);
	if (ctx->cgroup_map_fd < 0) {
	...
	/* Fork program */
	{
		int ppid_off = ek_bpf_tp_field_offset("sched",
			"sched_process_fork", "parent_pid");
		int cpid_off = ek_bpf_tp_field_offset("sched",
			"sched_process_fork", "child_pid");
		if (ppid_off >= 0 && cpid_off >= 0) {
			insn_cnt = generate_fork_program(prog, MAX_TP_INSNS,
				ctx->cgroup_map_fd, ctx->ringbuf_fd,
				ppid_off, cpid_off);
			if (insn_cnt > 0) {
				...
				if (ctx->prog_fds[EK_METRICS_PROG_FORK] >= 0)
					...
					LOG_WARN("metrics: fork prog load: %s",
						 strerror(-ctx->prog_fds[EK_METRICS_PROG_FORK]));
			}
		} else {
	...
	/* Exec program */
	{
		int pid_off = ek_bpf_tp_field_offset("sched",
			"sched_process_exec", "pid");
		if (pid_off >= 0) {
			insn_cnt = generate_exec_program(prog, MAX_TP_INSNS,
				ctx->cgroup_map_fd, ctx->ringbuf_fd,
				pid_off);
			if (insn_cnt > 0) {
				...
				if (ctx->prog_fds[EK_METRICS_PROG_EXEC] >= 0)
					...
					LOG_WARN("metrics: exec prog load: %s",
						 strerror(-ctx->prog_fds[EK_METRICS_PROG_EXEC]));
			}
		}
	}
	 ...
	 * comes from waitpid on Erlang side — not from tracepoint ctx) */
	{
		...
		if (pid_off >= 0) {
			insn_cnt = generate_exit_program(prog, MAX_TP_INSNS,
				ctx->cgroup_map_fd, ctx->ringbuf_fd,
				pid_off);
			if (insn_cnt > 0) {
				...
				if (ctx->prog_fds[EK_METRICS_PROG_EXIT] >= 0)
					...
					LOG_WARN("metrics: exit prog load: %s",
						 strerror(-ctx->prog_fds[EK_METRICS_PROG_EXIT]));
			}
		}
	}
	...
	/* OOM program */
	{
		int pid_off = ek_bpf_tp_field_offset("oom",
			"mark_victim", "pid");
		if (pid_off >= 0) {
			insn_cnt = generate_oom_program(prog, MAX_TP_INSNS,
				ctx->cgroup_map_fd, ctx->ringbuf_fd,
				pid_off);
			if (insn_cnt > 0) {
				...
				if (ctx->prog_fds[EK_METRICS_PROG_OOM] >= 0)
					...
					LOG_WARN("metrics: oom prog load: %s",
						 strerror(-ctx->prog_fds[EK_METRICS_PROG_OOM]));
			}
		}
	}
