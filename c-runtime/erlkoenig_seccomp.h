...
#define SC_CHECK_ARCH \
	SC_LOAD_ARCH, \
	SC_JUMP_EQ(AUDIT_ARCH_X86_64, 1, 0), \
	SC_KILL
#elif defined(__aarch64__)
#define SC_CHECK_ARCH \
	SC_LOAD_ARCH, \
	SC_JUMP_EQ(AUDIT_ARCH_AARCH64, 1, 0), \
	SC_KILL
#else
...
static struct sock_filter seccomp_strict[] = {
	...
	SC_JUMP_EQ(SYS_execve,         0, 1), SC_ALLOW,
	SC_JUMP_EQ(SYS_arch_prctl,     0, 1), SC_ALLOW,
	SC_JUMP_EQ(SYS_set_tid_address, 0, 1), SC_ALLOW,
	SC_JUMP_EQ(SYS_set_robust_list, 0, 1), SC_ALLOW,
	SC_JUMP_EQ(SYS_rseq,           0, 1), SC_ALLOW,
	SC_JUMP_EQ(SYS_prlimit64,      0, 1), SC_ALLOW,
	SC_JUMP_EQ(SYS_readlinkat,     0, 1), SC_ALLOW,
	/* I/O */
	SC_JUMP_EQ(SYS_read,           0, 1), SC_ALLOW,
	SC_JUMP_EQ(SYS_write,          0, 1), SC_ALLOW,
	SC_JUMP_EQ(SYS_readv,          0, 1), SC_ALLOW,
	SC_JUMP_EQ(SYS_writev,         0, 1), SC_ALLOW,
	SC_JUMP_EQ(SYS_close,          0, 1), SC_ALLOW,
	/* Process */
	SC_JUMP_EQ(SYS_exit,           0, 1), SC_ALLOW,
	SC_JUMP_EQ(SYS_exit_group,     0, 1), SC_ALLOW,
	SC_JUMP_EQ(SYS_rt_sigreturn,   0, 1), SC_ALLOW,
	SC_JUMP_EQ(SYS_rt_sigaction,   0, 1), SC_ALLOW,
	SC_JUMP_EQ(SYS_rt_sigprocmask, 0, 1), SC_ALLOW,
	/* Memory */
	SC_JUMP_EQ(SYS_brk,            0, 1), SC_ALLOW,
	SC_JUMP_EQ(SYS_mmap,           0, 1), SC_ALLOW,
	SC_JUMP_EQ(SYS_munmap,         0, 1), SC_ALLOW,
	SC_JUMP_EQ(SYS_mprotect,       0, 1), SC_ALLOW,
	/* Misc */
	SC_JUMP_EQ(SYS_clock_gettime,  0, 1), SC_ALLOW,
	SC_JUMP_EQ(SYS_clock_nanosleep, 0, 1), SC_ALLOW,
	SC_JUMP_EQ(SYS_nanosleep,      0, 1), SC_ALLOW,
	SC_JUMP_EQ(SYS_getrandom,      0, 1), SC_ALLOW,
	SC_JUMP_EQ(SYS_futex,          0, 1), SC_ALLOW,
	SC_JUMP_EQ(SYS_getpid,         0, 1), SC_ALLOW,
	SC_KILL,
};
...
static struct sock_filter seccomp_network[] = {
	...
	SC_JUMP_EQ(SYS_writev,         0, 1), SC_ALLOW,
	SC_JUMP_EQ(SYS_close,          0, 1), SC_ALLOW,
	SC_JUMP_EQ(SYS_dup2,           0, 1), SC_ALLOW,
	SC_JUMP_EQ(SYS_dup3,           0, 1), SC_ALLOW,
	/* Network */
	SC_JUMP_EQ(SYS_socket,         0, 1), SC_ALLOW,
	SC_JUMP_EQ(SYS_bind,           0, 1), SC_ALLOW,
	SC_JUMP_EQ(SYS_listen,         0, 1), SC_ALLOW,
	SC_JUMP_EQ(SYS_accept,         0, 1), SC_ALLOW,
	SC_JUMP_EQ(SYS_accept4,        0, 1), SC_ALLOW,
	SC_JUMP_EQ(SYS_connect,        0, 1), SC_ALLOW,
	SC_JUMP_EQ(SYS_sendto,         0, 1), SC_ALLOW,
	SC_JUMP_EQ(SYS_recvfrom,       0, 1), SC_ALLOW,
	SC_JUMP_EQ(SYS_sendmsg,        0, 1), SC_ALLOW,
	SC_JUMP_EQ(SYS_recvmsg,        0, 1), SC_ALLOW,
	SC_JUMP_EQ(SYS_setsockopt,     0, 1), SC_ALLOW,
	SC_JUMP_EQ(SYS_getsockopt,     0, 1), SC_ALLOW,
	SC_JUMP_EQ(SYS_getsockname,    0, 1), SC_ALLOW,
	SC_JUMP_EQ(SYS_getpeername,    0, 1), SC_ALLOW,
	SC_JUMP_EQ(SYS_shutdown,       0, 1), SC_ALLOW,
	/* Poll/epoll */
	SC_JUMP_EQ(SYS_poll,           0, 1), SC_ALLOW,
	SC_JUMP_EQ(SYS_ppoll,          0, 1), SC_ALLOW,
	SC_JUMP_EQ(SYS_epoll_create1,  0, 1), SC_ALLOW,
	SC_JUMP_EQ(SYS_epoll_ctl,      0, 1), SC_ALLOW,
	SC_JUMP_EQ(SYS_epoll_wait,     0, 1), SC_ALLOW,
	SC_JUMP_EQ(SYS_select,         0, 1), SC_ALLOW,
	/* Memory */
	SC_JUMP_EQ(SYS_brk,            0, 1), SC_ALLOW,
	SC_JUMP_EQ(SYS_mmap,           0, 1), SC_ALLOW,
	SC_JUMP_EQ(SYS_munmap,         0, 1), SC_ALLOW,
	SC_JUMP_EQ(SYS_mprotect,       0, 1), SC_ALLOW,
	SC_JUMP_EQ(SYS_mremap,         0, 1), SC_ALLOW,
	SC_JUMP_EQ(SYS_madvise,        0, 1), SC_ALLOW,
	/* Signals/process */
	SC_JUMP_EQ(SYS_exit,           0, 1), SC_ALLOW,
	SC_JUMP_EQ(SYS_exit_group,     0, 1), SC_ALLOW,
	SC_JUMP_EQ(SYS_rt_sigreturn,   0, 1), SC_ALLOW,
	SC_JUMP_EQ(SYS_rt_sigaction,   0, 1), SC_ALLOW,
	SC_JUMP_EQ(SYS_rt_sigprocmask, 0, 1), SC_ALLOW,
	SC_JUMP_EQ(SYS_sigaltstack,    0, 1), SC_ALLOW,
	/* File (read-only, no open with write) */
	SC_JUMP_EQ(SYS_openat,         0, 1), SC_ALLOW,
	SC_JUMP_EQ(SYS_fstat,          0, 1), SC_ALLOW,
	SC_JUMP_EQ(SYS_newfstatat,     0, 1), SC_ALLOW,
	SC_JUMP_EQ(SYS_lseek,          0, 1), SC_ALLOW,
	SC_JUMP_EQ(SYS_fcntl,          0, 1), SC_ALLOW,
	/*
	...
	/* Time */
	SC_JUMP_EQ(SYS_clock_gettime,  0, 1), SC_ALLOW,
	SC_JUMP_EQ(SYS_gettimeofday,   0, 1), SC_ALLOW,
	SC_JUMP_EQ(SYS_nanosleep,      0, 1), SC_ALLOW,
	SC_JUMP_EQ(SYS_clock_nanosleep, 0, 1), SC_ALLOW,
	/* Misc */
	SC_JUMP_EQ(SYS_getpid,         0, 1), SC_ALLOW,
	SC_JUMP_EQ(SYS_gettid,         0, 1), SC_ALLOW,
	SC_JUMP_EQ(SYS_getrandom,      0, 1), SC_ALLOW,
	SC_JUMP_EQ(SYS_futex,          0, 1), SC_ALLOW,
	SC_KILL,
};
...
static struct sock_filter seccomp_default[] = {
	...
	SC_JUMP_EQ(SYS_reboot,         0, 1), SC_KILL,
	SC_JUMP_EQ(SYS_kexec_load,     0, 1), SC_KILL,
	SC_JUMP_EQ(SYS_kexec_file_load, 0, 1), SC_KILL,
	SC_JUMP_EQ(SYS_init_module,    0, 1), SC_KILL,
	SC_JUMP_EQ(SYS_finit_module,   0, 1), SC_KILL,
	SC_JUMP_EQ(SYS_delete_module,  0, 1), SC_KILL,
	SC_JUMP_EQ(SYS_mount,          0, 1), SC_KILL,
	SC_JUMP_EQ(SYS_umount2,        0, 1), SC_KILL,
	SC_JUMP_EQ(SYS_pivot_root,     0, 1), SC_KILL,
	SC_JUMP_EQ(SYS_ptrace,         0, 1), SC_KILL,
	SC_JUMP_EQ(SYS_personality,    0, 1), SC_KILL,
	SC_JUMP_EQ(SYS_swapon,         0, 1), SC_KILL,
	SC_JUMP_EQ(SYS_swapoff,        0, 1), SC_KILL,
	SC_JUMP_EQ(SYS_sethostname,    0, 1), SC_KILL,
	SC_JUMP_EQ(SYS_setdomainname,  0, 1), SC_KILL,
	SC_JUMP_EQ(SYS_acct,           0, 1), SC_KILL,
	SC_JUMP_EQ(SYS_unshare,        0, 1), SC_KILL,
	SC_JUMP_EQ(SYS_setns,          0, 1), SC_KILL,
	SC_JUMP_EQ(SYS_keyctl,         0, 1), SC_KILL,
	SC_JUMP_EQ(SYS_add_key,        0, 1), SC_KILL,
	SC_JUMP_EQ(SYS_request_key,    0, 1), SC_KILL,
	SC_JUMP_EQ(SYS_bpf,            0, 1), SC_KILL,
	SC_JUMP_EQ(SYS_userfaultfd,    0, 1), SC_KILL,
	SC_JUMP_EQ(SYS_perf_event_open, 0, 1), SC_KILL,
...
static int erlkoenig_apply_seccomp(uint8_t profile)
{
	...
	case SECCOMP_PROFILE_DEFAULT:
		...
		filter_len = sizeof(seccomp_default) / sizeof(seccomp_default[0]);
		break;
	...
	case SECCOMP_PROFILE_NETWORK:
		...
		filter_len = sizeof(seccomp_network) / sizeof(seccomp_network[0]);
		break;
	default:
	 ...
	 */
	{
		...
		if (debug_env && debug_env[0] == '1') {
			for (size_t i = 0; i < filter_len; i++) {
				if (filter[i].code == (BPF_RET | BPF_K) &&
				    filter[i].k == SECCOMP_RET_KILL_PROCESS)
					filter[i].k = SECCOMP_RET_LOG;
			}
	...
	LOG_INFO("seccomp profile %u installed (%zu instructions)",
		 profile, filter_len);
	return 0;
}
